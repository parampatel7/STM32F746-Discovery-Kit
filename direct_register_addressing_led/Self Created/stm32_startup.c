...
//videolink: https://www.youtube.com/watch?v=2Hm8eEHsgls&list=PPSV
//channel name: Fastbit Embedded Brain Acadamy
#include <stdint.h>

// Define SRAM boundaries
#define SRAM_START  0x20000000U
#define SRAM_SIZE   (320U * 1024U)   // 320 KB
#define SRAM_END    (SRAM_START + SRAM_SIZE)
#define STACK_START SRAM_END         // Initial stack pointer value

// Function prototypes
void Reset_handler(void);
//weak: if NMI_handler defined in main.c by programmer it will be called instead of this below, main.c's NMI_handler will override this file's NMI_handler
void NMI_handler(void)    __attribute__((weak, alias("Default_handler"))); //as all irqs are not used hence for all 97of them no need to create individual functions
void HardFault_handler(void)    __attribute__((weak, alias("Default_handler")));


// Vector table placed in its own section
...
{
    // Normally, startup code and main() call go here
    while (1);
}
...
void Reset_Handler(void)
{
	...
	for(uint32_t i =0 ; i < size ; i++)
	{
		*pDst++ = 0;
        ...
        /* Call C library initialization (for constructors, etc.) */
	//__libc_init_array();
